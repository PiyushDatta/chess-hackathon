...
class MultiHeadSelfAttention(nn.Module):
    ...
    '''
    def __init__(self, embed_dim=512, nhead=8, head_dim=64, rope=True, causal=True, ghost=False, device='cpu', use_flash_attention=True):
        super().__init__()
        self.nhead = nhead
    ...
    def forward(self, inputs):
        ...
        attn_output = None
        if self.use_flash_attention and inputs.is_cuda:
            # Optionally apply rotary embeddings if available
            ...
            K = K.transpose(1, 2)
            V = V.transpose(1, 2)
            # Use built-in scaled_dot_product_attention.
            # Note: this function will use a Flash-Attention–like kernel under the hood on supported hardware.
            attn_output = torch.nn.functional.scaled_dot_product_attention(
                Q, K, V, attn_mask=None, dropout_p=0.0, is_causal=self.causal
            )
            # Transpose back to (batch_size, seq_len, nhead, head_dim)
            attn_output = attn_output.transpose(1, 2).contiguous()
            attn_output = attn_output.reshape(batch_size, seq_len, -1)
        else:
            attn_output = multihead_cross_attention(Q, K, V, self.rope_encoder, mask, self.ghost, self.device)
            attn_output = attn_output.reshape(batch_size, seq_len, -1)
        outputs = self.Wo(attn_output)
        return outputs
